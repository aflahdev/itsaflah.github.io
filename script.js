...
if (typeof PAGE_DATA !== 'undefined') {
  // Render Pills
  const pillsContainer = document.getElementById('social-pills');
  if (pillsContainer) {
    pillsContainer.innerHTML = PAGE_DATA.socialPills.map(p => `
      <a href="${p.href}" class="pill">
        ${p.icon}${p.text}
      </a>
    `).join('');
  }
  ...
  const renderCards = (containerId, cards) => {
    ...
    container.innerHTML = cards.map(c => `
      <a href="${c.href}" class="card" data-d="${c.delay}"${c.ariaLabel ? ` aria-label="${c.ariaLabel}"` : ''}>
        <div class="ci ${c.colorClass}">${c.icon}</div>
        <div class="cb">
          <div class="cn">${c.title}</div>
          <div class="cd">${c.desc}</div>
        </div>
        ${c.badgeText ? `<div class="badge ${c.badgeClass}">${c.badgeText}</div>` : ''}
        <div class="ca">→</div>
      </a>
    `).join('');
  };

  renderCards('social-cards', PAGE_DATA.socialCards);
  renderCards('project-cards', PAGE_DATA.projectCards);
  renderCards('writing-cards', PAGE_DATA.writingCards);
}
